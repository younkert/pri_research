...
<br />
<br />
Any acronyms or ambiguous abbreviations can be resolved by looking at the Swagger API for cBioPortal <http://oncotree.mskcc.org/cdd/swagger-ui.html#/clinical-data-dictionary-controller>
...
full_data[, case_id := gsub(pattern = ".01", replacement = "", x = case_id)]
```
<br />
<br />
...
### Survival Analysis
This data comes with information regarding several types of survival and other time series data amenable to Kaplan Meier analysis. This section of code examines these data. We can abritrarily divide the data into groups and use their common survival metric to see which group last longer than the other.
```{r surv_al, fig.width=10}
#' [Load packages]
p_load(survival, survminer)

#' [Input and format survival data]
OS_surv_raw <- fread(file = "/Users/Teddy/R/repos/pri_research/CD91_Sarcoma_Expression/data/input/Overall_Survival.txt")
DS_surv_raw <- fread(file = "/Users/Teddy/R/repos/pri_research/CD91_Sarcoma_Expression/data/input/Disease_Free_Survival.txt")

expression_attributes <- full_data[,.(case_id, gene, expr_TPM, Median, Tertile, Pentile)]
expression_attributes <- dcast(data = expression_attributes, 
                               formula = case_id+Median+Tertile+Pentile~gene, 
                               value.var = "expr_TPM")

#' @change names 
setnames(x = OS_surv_raw, 
         old = names(OS_surv_raw), 
         new = c("case_id", "study_id", "number_at_risk", "status", "survival_rate", "time_(months)"))
setnames(x = DS_surv_raw, 
         old = names(DS_surv_raw), 
         new = c("case_id", "study_id", "number_at_risk", "status", "survival_rate", "time_(months)"))

#' @alter case_id's
OS_surv_raw[, case_id := gsub(pattern = "-", replacement = ".", x = case_id)]
DS_surv_raw[, case_id := gsub(pattern = "-", replacement = ".", x = case_id)]

#' @merge in expression attributes to the survival data
OS_surv <- merge(x = OS_surv_raw, y = expression_attributes, by = "case_id", all.x = TRUE)
DS_surv <- merge(x = DS_surv_raw, y = expression_attributes, by = "case_id", all.x = TRUE)

#' @Remove uneeded previous survival data
OS_surv[, c("number_at_risk", "survival_rate") := NULL]
DS_surv[, c("number_at_risk", "survival_rate") := NULL]

#' @Classify status bases on [1: censored] and [2: dead]
OS_surv[status == "censored", status := "1"]
DS_surv[status == "censored", status := "1"]
OS_surv[status == "deceased", status := "2"]
DS_surv[status == "deceased", status := "2"]
#Change to numeric
OS_surv[, status := as.numeric(status)]
DS_surv[, status := as.numeric(status)]

#' @SURV object creation


s <- Surv(OS_surv$`time_(months)`, OS_surv$status)
survfit(s~1)
survfit(Surv(`time_(months)`, status)~1, data=OS_surv)
sfit <- survfit(Surv(`time_(months)`, status)~1, data=OS_surv)
sfit
summary(sfit)

#' @Plot
ggplot(data = OS_surv, aes(x = OS_surv$`time_(months)`, y = OS_surv$survival_rate)) +
  geom_point() +
  theme_bw()


scatterPlot0 <- ggplot(OS_surv, aes(x = time_, y = finalUCEC$survivalEstimate, color = project_id)) + 
...
#### Data Summary Barcharts
```{r barcharts}
library(ggrepel)
extr_data <- full_data[,.(case_id, AGE, ANEUPLOIDY_SCORE, CANCER_TYPE_DETAILED, 
                          ETHNICITY, FRACTION_GENOME_ALTERED, MUTATION_COUNT,
                          NEW_TUMOR_EVENT_AFTER_INITIAL_TREATMENT, RACE, SEX,
                          TUMOR_TISSUE_SITE, TUMOR_TYPE, expr_TPM, gene)]

#' @AGE plot
ggplot(data = extr_data, mapping = aes(AGE)) +
  geom_bar() +
  theme_bw() +
  ylim(0,70) +
  scale_x_continuous(breaks=seq(20,100,5)) +
  theme(axis.text.x = element_text(angle = 60, hjust = 1)) + 
  geom_text_repel(stat = "count", mapping = aes(label = ..count.., color = "red"), vjust = -1) +
  ggtitle("Age Distribution of Sarcoma Cases with LRP1 Data") +
  ylab("Count") +
  xlab("Age")

#' @ANEUPLOIDY_SCORE plot
ggplot(data = extr_data, mapping = aes(ANEUPLOIDY_SCORE)) +
  geom_bar() +
  theme_bw() +
  ylim(0,80) +
  theme(axis.text.x = element_text(angle = 60, hjust = 1)) + 
  geom_text_repel(stat = "count", mapping = aes(label = ..count.., color = "red"), vjust = -1, show.legend = FALSE) +
  ggtitle("Aneuploidy Score Distribution of Sarcoma Cases with LRP1 Data") +
  ylab("Count") +
  xlab("Aneuploidy Scores")

#' @CANCER_TYPE
ggplot(data = extr_data, mapping = aes(CANCER_TYPE_DETAILED)) +
  geom_bar(aes(group = CANCER_TYPE_DETAILED, fill = CANCER_TYPE_DETAILED)) +
  theme_bw() +
  ylim(0,550) +
  theme(axis.text.x = element_blank(), legend.position = "bottom", legend.title = element_blank()) + 
  geom_text_repel(stat = "count", mapping = aes(label = ..count.., color = "red"), vjust = -1, show.legend = FALSE) +
  ggtitle("Cancer Types of Sarcoma Cases with LRP1 Data") +
  ylab("Count") +
  xlab("Cancer Types") +
  scale_fill_brewer(palette = 1)

#' @SEX
ggplot(data = extr_data, mapping = aes(SEX)) +
  geom_bar(aes(group = SEX, fill = SEX), show.legend = FALSE) +
  theme_bw() +
  geom_text_repel(stat = "count", mapping = aes(label = ..count.., color = "red"), vjust = -1, show.legend = FALSE) +
  ggtitle("Sexes of Sarcoma Cases with LRP1 Data") +
  ylab("Count") +
  xlab("Sexes") +
  scale_fill_brewer(palette = 1, direction = -1)

#' @TUMOR_TISSUE_SITE
ggplot(data = extr_data, mapping = aes(TUMOR_TISSUE_SITE)) +
  geom_bar(aes(group = TUMOR_TISSUE_SITE, fill = TUMOR_TISSUE_SITE), show.legend = FALSE) +
  theme_bw() +
  ylim(0,400) +
  geom_text_repel(stat = "count", mapping = aes(label = ..count.., color = "red"), vjust = -1, show.legend = FALSE) +
  ggtitle("Tumor Tissue Sites of Sarcoma Cases with LRP1 Data") +
  ylab("Count") +
  xlab("Tissue Sites") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

#' @TUMOR_TYPE
ggplot(data = extr_data, mapping = aes(TUMOR_TYPE)) +
  geom_bar(aes(TUMOR_TYPE), show.legend = FALSE) +
  theme_bw() +
  ylim(0,400) +
  geom_text_repel(stat = "count", mapping = aes(label = ..count.., color = "red"), vjust = -1, show.legend = FALSE) +
  ggtitle("Tumor Types of Sarcoma Cases with LRP1 Data") +
  ylab("Count") +
  xlab("Tumor Types") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

#' @MUTATION_COUNT
ggplot(data = extr_data, mapping = aes(MUTATION_COUNT)) +
  geom_bar(aes(MUTATION_COUNT), show.legend = FALSE) +
  theme_bw() +
  #geom_text_repel(stat = "count", mapping = aes(label = ..count.., color = "red"), vjust = -1, show.legend = FALSE) +
  ggtitle("Mutation Count Distribution of Sarcoma Cases with LRP1 Data") +
  ylab("Count") +
  xlab("Mutation Count") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

#' @NEW_TUMOR_EVENT_AFTER_INITIAL_TREATMENT
ggplot(data = extr_data, mapping = aes(NEW_TUMOR_EVENT_AFTER_INITIAL_TREATMENT)) +
  geom_bar(aes(NEW_TUMOR_EVENT_AFTER_INITIAL_TREATMENT), show.legend = FALSE) +
  theme_bw() +
  ylim(0,650) +
  geom_text_repel(stat = "count", mapping = aes(label = ..count.., color = "red"), vjust = -1, show.legend = FALSE) +
  ggtitle("# New Tumor Events after Initial Treatment of Sarcoma Cases with LRP1 Data") +
  ylab("Count") +
  xlab("New Tumor Events Post Initial Treatment") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
#### Data Summary Scatterplots
This section of code looks at the continuous variable of LRP1 expression count and any available continuous variable in the dataset.
```{r scatters}

#' @AGE
ggplot(data = extr_data[gene == "LRP1"], mapping = aes(x = expr_TPM, y = AGE)) +
  geom_point(shape = 21) +
  ggtitle("LRP1 expression vs Age") + 
  theme_bw()

#' @ANEUPLOIDY_SCORE
ggplot(data = extr_data[gene == "LRP1"], mapping = aes(x = expr_TPM, y = ANEUPLOIDY_SCORE)) +
  geom_point(shape = 21) +
  ggtitle("LRP1 expression vs Aneuploidy Score") + 
  theme_bw()

#' @CANCER_TYPE_DETAILED
ggplot(data = extr_data[gene == "LRP1"], mapping = aes(x = CANCER_TYPE_DETAILED, y = expr_TPM)) +
  geom_violin(aes(goup = CANCER_TYPE_DETAILED, color = CANCER_TYPE_DETAILED), show.legend = FALSE) +
  geom_jitter(position = position_jitter(0.1), shape = 21) +
  ggtitle("LRP1 expression and Cancer Type Detailed") + 
  theme_bw() +
  theme(axis.text.x = element_text(angle = 20, hjust = 1))

#' @FRACTION_GENOME_ALTERED
ggplot(data = extr_data[gene == "LRP1"], mapping = aes(x = expr_TPM, y = FRACTION_GENOME_ALTERED)) +
  geom_point(shape = 21) +
  theme_bw() +
  ggtitle("Fraction of Genome Altered vs LRP1 Expression Count")

#' @MUTATION_COUNT
ggplot(data = extr_data[gene == "LRP1"], mapping = aes(x = expr_TPM, y = MUTATION_COUNT)) +
  geom_point(shape = 21) +
  theme_bw() +
  ggtitle("Mutation Count vs LRP1 Expression Count")

#' @TUMOR_TISSUE_SITE
ggplot(data = extr_data[gene == "LRP1"], mapping = aes(x = TUMOR_TISSUE_SITE, y = expr_TPM)) +
  geom_jitter(position = position_jitter(0.1), shape = 21) +
  ggtitle("LRP1 expression and Tumor Tissue Site") + 
  theme_bw() +
  theme(axis.text.x = element_text(angle = 30, hjust = 1))

#' @TUMOR_TYPE
ggplot(data = extr_data[gene == "LRP1"], mapping = aes(x = TUMOR_TYPE, y = expr_TPM)) +
  geom_jitter(position = position_jitter(0.1), shape = 21) +
  ggtitle("LRP1 expression and Tumor Type") + 
  theme_bw() +
  theme(axis.text.x = element_text(angle = 30, hjust = 1))
